...
## 71. 2026-07-18 设备会话去重与聊天状态栏脱敏精简

- 根因确认：每次 PKCE 登录都会创建新的设备会话；旧 Gateway 未按同一设备名称和平台撤销旧会话，
  所以同一 Windows 电脑会显示多个“有效”设备。
- 隔离 Gateway 现在会在同一账号、同一设备名称和平台重新登录时撤销此前活跃会话，设备列表只返回
  当前有效会话；不同名称或平台的设备不受影响。新增重复登录回归测试。
- 聊天输入框状态栏不再展示邮箱或当前模型；已调整为：
  `AI 服务正常 · 剩余额度 <积分> · 已使用 <百分比>`。
...
## 73. 2026-07-18 US4 组织与分级管理基础

- 已开始 T060–T068。首先交付统一的 Gateway 服务端授权策略并增加 4 项合同测试：
  一级管理员全局权限、二级管理员组织范围、二级管理员仅可管理本组织普通用户，以及最后一个
  有效一级管理员保护。
...
## 77. 2026-07-18 邀请码截止时间与账号 AI 权限到期语义修正

- 用户确认：邀请码页面中的截止时间不是仅控制“邀请码还能否注册”，而是通过该邀请码
  注册的用户可以使用 AI 的权限截止时间。
- 已统一为单一时间语义：
  - 用户必须在 `invitation.expiresAt` 之前完成注册；
  - 注册成功时 `account.expiresAt` 原子写入同一个时间；
  - 截止时间到达后，现有账号状态、登录、Token 刷新和新 Turn 门禁沿用账号到期检查，
    不允许继续使用 AI；
  - 撤销邀请码只阻止后续注册，不会把已注册账号提前改成其他截止时间。
- Gateway 分支已提交并推送 `214e949`
  （`fix(auth): bind invitation deadline to account expiry`）。
- 管理页面文案已改为“AI 权限截止时间”，并明确提示：
  “用户须在该时间前完成注册；注册账号的 AI 使用权限也将在该时间到期。”
  邀请码列表和一次性显示提示也改为展示账号 AI 权限截止时间。
- 新增回归断言：普通注册与并发最后一次注册成功后，数据库中的
  `accounts.expires_at` 必须严格等于对应 `invitations.expires_at`。
- `spec.md`、`data-model.md`、`contracts/admin-api.md` 和
  `contracts/auth-account-api.md` 已同步这一语义，避免客户端和服务器再次产生不同理解。
...
## 103. 2026-07-21 Quick Tunnel expiry diagnosis and fail-fast preview launcher

- The previous preview origin `hour-hosts-sega-troy.trycloudflare.com` now returns Cloudflare HTTP 530 / Error 1016; the local Gateway was not the cause.
