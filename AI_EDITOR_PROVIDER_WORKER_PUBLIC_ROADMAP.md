...
`2026-07-21 implementation checkpoint`: T139 已在
`codex/provider-worker-mvp@6e73100` 完成。30 个名额按全部已准入产品账号计算，
包括初始化管理员、一级/二级管理员和普通用户；容量预留与邀请码消费、账号创建使用
同一数据库事务。管理端没有环境变量或 API 绕过开关，第 31 个账号继续由长期架构
验收门禁阻止。
