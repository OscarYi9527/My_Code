...
- T048/T090 的 Code 测试框架已提前完成：覆盖启动加载、手动刷新、动态新增模型、富
  元数据、去重、401/失败时清空旧目录，以及登录或服务恢复后的重新加载。任务仍保持
  未勾选，直到 Black 的真实 Gateway-backed Edge 可用于联合验证。
- T112/T113 的自动化基础设施已提前完成：统一脚本启动并清理隔离
