...
- **FR-036**: 一级管理员 MUST 能设置组织月度总积分；二级管理员 MUST 能在组织范围内
  设置本组织用户可用积分。一级管理员账号自身 MUST 不参与个人积分、透支或累计风险限制，
  其账号状态和管理页面 MUST 明确显示“额度不受限”，不得显示为零余额。
...
- **FR-058**: 公网 MVP 的短期架构 MUST 将用户数限制为 30；加入第 31 个用户前 MUST
  ...
  双 Worker/热备 Worker、集中监控和故障切换验收。该限制按已准入的产品账号总数
  计算，一级管理员、二级管理员、普通用户和初始化管理员均计入 30 个名额；禁用账号
  不得静默释放名额。容量预留、邀请码消费和账号创建 MUST 位于同一数据库事务内，
  第 31 个账号注册失败时不得消耗邀请码次数。
