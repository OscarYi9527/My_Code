...
2. **Given** 二级管理员，**When** 创建邀请码，**Then** 邀请码固定绑定本组织并带有效期
   和使用次数；二级管理员不能创建跨组织邀请码。
...
- **FR-032**: 二级管理员 MUST 能创建绑定本组织、带有效期和使用次数的邀请码；一级
  管理员 MUST 能查看和撤销全部邀请码。
